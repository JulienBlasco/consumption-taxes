...
* modified 16 November to add comparison of models
* modified 23 Novembre to add summaries
* modifier 15 March to add correlation and R2
*******************
...
program preprocessing     
 ...
 noisily di "- " c(current_time)  
		...
		noisily glm hmc_medianized c.log_dhi_medianized [aw=hwgt*nhhmem]  if scope_regression, link(log)
		 predict hmc_medianized_predict0 if scope
		...
		i.nhhmem_top i.hpartner_agg [aw=hwgt*nhhmem]  if scope_regression, link(log)  
		 predict hmc_medianized_predict1 if scope
		...
		$depvars [aw=hwgt*nhhmem]  if scope_regression, link(log)  
		 predict hmc_medianized_predict2 if scope
...
multiple_csv $ccyy_to_imput, test summaries model(2)
