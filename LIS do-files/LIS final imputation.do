...
program variables_creation
	...
	foreach def in carey euro ours {   
	 ...
	 // version with rent  
	 egen hmc_medianized_predict_mean = wtmean(hmc_medianized_predict) if scope, by(ccyy)  weight(hwgt) 
	 egen hmc_medianized_predict_2 = hmc_medianized_predict ^ 2
	 egen hmc_medianized_predict_2_mean = wtmean(hmc_medianized_predict_2) if scope, by(ccyy) weight(hwgt)
	 
	 gen beta_pred = (oecd_prop * dhi_mean - hmc_medianized_predict_mean)/hmc_medianized_predict_2_mean
	 
	 gen hmc_pred_scaled = hmc_medianized_predict * (1+beta_pred*hmc_medianized_predict)
